...
/**
 * A collection of small utilities for pretty printing nodes, triples and
 * associated things. The core functionality here is a static prefix map which
 * is preloaded with known prefixes.
 */
public class PrintUtil {
	...
	public static String print(Node node) {
		if (node instanceof Node_URI) {
			...
			return "'" + lf + "'"
					+ (TermUtil.isSimpleString(node) ? "" : "^^" + node.getLiteralDatatypeURI());
	 ...
	 */
	public static String print(RDFObject node) {
		if (node == null)
			return "null";
	...
	public static String print(Triple triple) {
		if (triple == null)
			return "(null)";
		return "(" + print(triple.getSubject()) + " @" + print(triple.getPredicate()) + " "
	...
	public static String print(TriplePattern triple) {
		...
		String ret = "(" + print(triple.getSubject()) + " @" + print(triple.getPredicate()) + " "
				+ print(triple.getObject()) + ")";

		if (triple.isStableTruth())
			return "!" + ret;
		else
			return ret;
	}
	...
	/**
	 * Expand qnames to URIs. If the given URI appears to start with one of the
	 * registered prefixes then expand the prefix, otherwise return the original URI
	 */
