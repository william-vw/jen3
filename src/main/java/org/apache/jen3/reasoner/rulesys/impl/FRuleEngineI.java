...
/**
 * Rule engines implement the internals of forward rule inference graphs and the
 * forward part of hybrid graphs. This interface abstracts the interface onto
 * such engines to allow a graph to switch between direct and RETE style
 * implementations.
 */
public interface FRuleEngineI {
	 ...
	 * already be preprocessed and the rules have been compiled
	 * 
	...
	/**
	 * Process rules that infer the given predicates. This requires the N3ModelSpec
	 * to have been setup with these predicates (allowRederiveFor) so rules with
	 * these predicates can be indexed.
	 * 
	 * @param predicates
	 */
	...
	/**
	 * Add a rule to this engine. The rule will be compiled (e.g., added to internal
	 * indices) but no reasoning will take place.
	 * 
	 * @param rule
	 */
	...
	/**
	 * Remove a rule from this engine. Cleanup will occur (e.g., removal from
	 * internal indices) but no reasoning will take place.
	 * 
	 * @param rule
	 */
