 ...
 * are not yet used.
 * <p>
 * Note that this should not be used in a real Triple, in particular it should
 * not end up in a Graph. It is only needed for the rule systems.
 * </p>
 */
public class Node_RuleVariable extends Node_Variable {
	...
	/**
	 * The value to which this variable is bound, can be another variable, itself
	 * (meaning unbound) or an actual value
	 */
	...
	/**
	 * A rule variable will replace quick-vars and blank nodes inside rules, but the
	 * former cannot exist in a graph. When needed to round-trip, replace again with
	 * the original replaced node.
	 **/
	...
	/**
	 * Return an indexable object for this Node. This is actually the rule label.
	 * This is weird but needed because equals is (deliberately) perverse on
	 * Node_RuleVariable so if we want to put then in a Set or Map we need something
	 * with a better equals function.
	 */
...
//    }
	...
	/**
	 * Binds a value to the brule version of the variable. Does not follow any
	 * reference trail, assues we have already been derefenced.
	 * 
	 * @param node a concrete Node value or another Node_RuleVariable to alias to
	 */
	 ...
	 * 
	 * @return either a concrete node value or the last variable in the reference
	 *         chain.
	 */
	public Node deref() {
	...
	/**
	 * Clone the rule variable to allow multiple rule instaces to be active at the
	 * same time.
	 */
	...
	public Node visitWith(NodeVisitor v, Object data) {
		return v.visitRuleVariable(this, data);
	}
	...
	@Override
	public String toString(PrefixMapping pm, boolean b) {
		return toString();
	}
