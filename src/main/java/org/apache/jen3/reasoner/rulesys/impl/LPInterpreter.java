...
import org.apache.jen3.reasoner.rulesys.BackwardRuleInfGraphI;
...
import org.apache.jen3.reasoner.rulesys.RuleDerivation;
import org.apache.jen3.reasoner.rulesys.impl.RuleClauseCode.CompileState.RuleClauseCodeList;
import org.apache.jen3.util.PrintUtil;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Bytecode interpreter engine for the LP version of the backward chaining rule
 * system. An instance of this is forked off for each parallel query.
 */
public class LPInterpreter {
	...
	/**
	 * Trick to allow the very top level triple lookup to return results with
	 * reduced store turnover
	 */
	 ...
	 * @param engine the engine which is calling this interpreter
	 * @param goal   the query to be satisfied
	 * @param isTop  true if this is a top level call from the outside iterator,
	 *               false means it is an internal generator call which means we
	 *               don't need to insert an tabled call
	 */
	 ...
	 *                don't need to insert an tabled call
	 */
	public LPInterpreter(LPBRuleEngine engine, TriplePattern goal, List<RuleClauseCode> clauses,
			boolean isTop) {
		this.engine = engine;
		this.goal = goal; // Used for debug only

		// Construct dummy top environemnt which is a call into the clauses for this
		// goal
...
//        TripleMatchFrame tmFrame = new TripleMatchFrame(this);
	...
	/**
	 * Called by top level interpreter to set to execution context for this
	 * interpreter to be top level instead of an internal generator.
	 */
	...
	/**
	 * Return the next result from this engine, no further initialization. Should be
	 * called from within an appropriately synchronized block.
	 */
	public Object next() {
...
//        System.out.println("end next() on interpreter for goal " + goal);
	...
	/**
	 * Return the context in which this interpreter is running, that is either the
	 * Generator for a tabled goal or a top level iterator.
	 */
	...
	/**
	 * Restore the current choice point and restart execution of the LP code until
	 * either find a successful branch (in which case exit with StateFlag.ACTIVE and
	 * variables bound to the correct results) or exhaust all choice points (in
	 * which case exit with StateFlag.FAIL and no bound results). In future tabled
	 * version could also exit with StateFlag.SUSPEND in cases whether the
	 * intepreter needs to suspend to await tabled results from a parallel proof
	 * tree.
	 */
		...
		main: while (cpFrame != null) {
			...
			if (cpFrame instanceof ChoicePointFrame) {
				...
				if (!choice.hasNext()) {
					...
					if (traceOn)
						logger.info(
								"FAIL in clause " + choice.envFrame.clause + " choices exhausted");
					continue main;
				}
			...
			interpreter: while (envFrame != null) {
				...
				while (true) {
					...
					case RuleClauseCode.CALL_PREDICATE:
						...
						boolean groundCall = isGrounded(argVars[0]) && isGrounded(argVars[1])
								&& isGrounded(argVars[2]);
					...
					case RuleClauseCode.PROCEED:
						...
						if (recordDerivations && envFrame.getRule() != null) {
							if (envFrame instanceof EnvironmentFrameWithDerivation) {
								...
								RuleDerivation d = new RuleDerivation(envFrame.getRule(), result,
										matches, infGraph);
	 ...
	 */
	private void setupClauseCall(int pc, int ac, List<RuleClauseCode> clauses,
			boolean isSingleton) {
	...
	public static Node derefPossFunctor(Node node) {
		if (node instanceof Node_RuleVariable) {
			...
			if (dnode.isVariable()) {
				...
				throw new ReasonerException(
						"Internal error in LP reasoner: variable in triple result");
			}
	...
	/**
	 * Standardize a node by replacing instances of wildcard ANY by new distinct
	 * variables. This is used in constructing the arguments to a top level call
	 * from a goal pattern.
	 * 
