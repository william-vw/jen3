...
import org.apache.jen3.util.Tokenizer;
...
// accessors
	...
	public void forEachInBody(BiConsumer<ClauseEntry, Integer> consumer) {
		for (int i = 0; i < body.length; i++)
			consumer.accept(body[i], i);
	}
	...
	public void forEachInHead(Consumer<ClauseEntry> consumer) {
		for (int i = 0; i < head.length; i++)
			consumer.accept(head[i]);
	}
	...
	private Node cloneNode(Node nIn, Map<Node_RuleVariable, Node> vmap, BindingEnvironment env) {
		Node n = (env == null) ? nIn : env.ground(nIn);
