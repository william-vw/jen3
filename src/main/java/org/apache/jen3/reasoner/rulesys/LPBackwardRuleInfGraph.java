...
import org.apache.jen3.reasoner.rulesys.impl.BBRuleContext;
import org.apache.jen3.reasoner.rulesys.impl.BindingVector;
import org.apache.jen3.reasoner.rulesys.impl.LPBRuleEngine;
import org.apache.jen3.reasoner.rulesys.impl.LPRuleStore;
import org.apache.jen3.reasoner.rulesys.impl.TempNodeCache;
...
import org.apache.jen3.util.iterator.UniqueFilter;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Inference graph for accessing the LP version of the backward chaining rule
 * engine.
 */
...
//   Core methods
	...
	/**
	 * Constructor. Create a new backward inference graph to process the given data.
	 * The parent reasoner supplies the ruleset and any additional schema graph.
	 * 
	 ...
	 */
	public LPBackwardRuleInfGraph(Reasoner reasoner, LPRuleStore ruleStore, Graph data,
			Graph schema, GraphConfig config) {
	 ...
	 */
	@Override
	public synchronized ExtendedIterator<Triple> findWithContinuation(TriplePattern pattern,
			Finder continuation) {
		checkOpen();
		this.requirePrepared();
		ExtendedIterator<Triple> result = engine.find(pattern)
				.filterKeep(new UniqueFilter<Triple>());
		if (continuation != null) {
			result = result.andThen(continuation.find(pattern));
		}
		return result; // result.filterDrop(Functor.acceptFilter);
	}
...
//     Interface between infGraph and the goal processing machinery
	...
	/**
	 * Match a pattern just against the stored data (raw data, schema, axioms) but
	 * no derivation.
	 */
	@Override
	...
	public boolean processBuiltin(ClauseEntry clause, Rule rule, BindingEnvironment env) {
		throw new ReasonerException(
				"Internal error in FBLP rule engine, incorrect invocation of building in rule "
						+ rule);
	}
...
//     Rule engine extras
	...
	/**
	 * Find any axioms (rules with no body) in the rule set and add those to the
	 * auxiliary graph to be included in searches.
	 */
				...
				for (int j = 0; j < rule.headLength(); j++) {
					...
					if (axiom instanceof TriplePattern) {
						...
						if (implementation == null) {
							throw new ReasonerException(
									"Attempted to invoke undefined functor: " + f);
						}
						Node[] args = f.getArgs();
