...
import org.apache.jen3.reasoner.rulesys.impl.OWLRuleTranslationHook;
import org.apache.jen3.vocabulary.ReasonerVocabulary;

/**
 * A reasoner interface that is able to invoke any of the useful rule engine
 * combinations. The rule set can be set after the reasoner instance is created.
 * The mode can be set to forward, backward or hybrid. The OWL-specific rule
 * augmentation can be included. Each of these settings can be controlled using
 * the configuration graph, specific methods calls or generic setParameter
 * calls.
 */
public class GenericRuleReasoner extends FBRuleReasoner {
	...
	/**
	 * Flag, if true then subClass and subProperty lattices will be optimized using
	 * TGCs, only applicable to HYBRID reasoners
	 */
	...
	/**
	 * Optional set of preprocessing hooks to be run in sequence during preparation
	 * time, only applicable to HYBRID modes
	 */
	...
	/**
	 * Constant - the mode description for mixed forward/backward, this is the
	 * default mode
	 */
	public static final RuleMode HYBRID = new RuleMode("hybrid");

	/**
	 * Constant - the mode description for mixed N3 forward/backward
	 */
	public static final RuleMode HYBRID_N3 = new RuleMode("hybrid-n3");
...
//  Constructors
	 ...
	 * ReasonerFactory and so has no capabilities description.
	 * 
	 ...
	 * Constructor
	 * 
	 * @param factory       the parent reasoner factory which is consulted to answer
	...
	/**
	 * Internal constructor, used to generated a partial binding of a schema to a
	 * rule reasoner instance.
	 */
	protected GenericRuleReasoner(List<Rule> rules, Graph schemaGraph, ReasonerFactory factory,
			RuleMode mode) {
		this(rules, factory);
...
//  Parameter control
	...
	/**
	 * Set the direction of rule inference desired. If set to a pure mode (FORWARD,
	 * BACKWARD) then the rules will be interpreted as operating in that direction
	 * which ever direction they were written in. In HYBRID mode then the direction
	 * of the rule itself which control whether it is used in forward or backward
	 * mode. In addition, HYBRID mode allows forward rules to generate addition
	 * backward rules.
	 */
	...
	/**
	 * Set (or change) the rule set that this reasoner should execute. This will not
	 * affect inference models already created from this reasoner.
	 * 
	...
	/**
	 * Set to true to enable translation of selected parts of an OWL schema to
	 * additional rules. At present only intersection statements are handled this
	 * way. The translation is only applicable in HYBRID mode.
	 */
	 ...
	 * can only work correct with some rule sets. This is only guaranteed to be
	 * implemented for the HYBRID mode.
	 */
	...
	/**
	 * Add a new preprocessing hook defining an operation that should be run when
	 * the inference graph is being prepared. This can be used to generate
	 * additional data-dependent rules or translations. This is only guaranteed to
	 * be implemented for the HYBRID mode.
	 */
	...
	@Override
	protected boolean doSetResourceParameter(Resource parameter, Resource value) {
	...
	private boolean isHasRule(Resource parameter) {
		return parameter.equals(ReasonerVocabulary.hasRule);
	}

	private boolean isRuleSet(Resource parameter) {
		return parameter.equals(ReasonerVocabulary.ruleSet);
	}

	private boolean isRuleSetURL(Resource parameter) {
		return parameter.equals(ReasonerVocabulary.ruleSetURL);
	}

	/**
	 * Internal version of setParameter that does not directly raise an exception on
	 * parameters it does not recognize.
	 * 
	 * @return false if the parameter was not recognized
	 */
	@Override
	protected boolean doSetParameter(Resource parameter, Object value) {
		if (parameter.equals(ReasonerVocabulary.PROPenableFunctorFiltering)) {
			...
			if (value.equals(FORWARD.name)) {
				...
				mode = HYBRID;
			} else if (value.equals(HYBRID_N3.name)) {
				mode = HYBRID_N3;
			} else {
				throw new IllegalParameterException(
						"PROPruleMode can only be 'forward'm 'forwardRETE', 'backward', 'hybrid', not "
								+ value);
			}
			...
			if (value instanceof String) {
				...
				throw new IllegalParameterException(
						"PROPruleSet value should be a URI string. Was a " + value.getClass());
			}
		} else {
...
//  Implementation methods
	...
	public Reasoner bindSchema(Graph tbox) throws ReasonerException {
		...
		Graph graph = null;
		if (mode == FORWARD) {
			graph = new BasicForwardRuleInfGraph(this, rules, null, tbox, tbox.getConfig());
			((InfGraph) graph).prepare();
		} else if (mode == FORWARD_RETE) {
			graph = new RETERuleInfGraph(this, rules, null, tbox, tbox.getConfig());
			((InfGraph) graph).prepare();
		} else if (mode == BACKWARD) {
			graph = tbox;
		} else if (mode == HYBRID_N3) {
			N3FBRuleInfGraph fbgraph = new N3FBRuleInfGraph(this, rules, getPreload(), tbox,
					tbox.getConfig());
			fbgraph.setTraceOn(traceOn);
			graph = fbgraph;
		} else {
			List<Rule> ruleSet = rules;
			graph = new FBRuleInfGraph(this, ruleSet, getPreload(), tbox, tbox.getConfig());
			if (enableTGCCaching)
	...
	public InfGraph bind(Graph data) throws ReasonerException {
		return bind(data, null);
	}

	public InfGraph bind(Graph data, DeductionListener listener) throws ReasonerException {
		Graph schemaArg = schemaGraph == null ? getPreload() : schemaGraph;
		InfGraph graph = null;
		if (mode == FORWARD) {
			graph = new BasicForwardRuleInfGraph(this, rules, schemaArg, data.getConfig());
			((BasicForwardRuleInfGraph) graph).setTraceOn(traceOn);
		} else if (mode == FORWARD_RETE) {
			graph = new RETERuleInfGraph(this, rules, schemaArg, data.getConfig());
			((BasicForwardRuleInfGraph) graph).setTraceOn(traceOn);
			...
			N3FBRuleInfGraph fbgraph = new N3FBRuleInfGraph(this, rules, schemaArg,
					data.getConfig());
			fbgraph.setTraceOn(traceOn);
			graph = fbgraph;
		} else {
			List<Rule> ruleSet = ((FBRuleInfGraph) schemaArg).getRules();
			FBRuleInfGraph fbgraph = new FBRuleInfGraph(this, ruleSet, schemaArg, data.getConfig());
			graph = fbgraph;
			if (enableTGCCaching)
		...
		graph.rebind(data);
		if (listener != null)
			graph.setDeductionListener(listener);
		return graph;
	}
	...
	protected synchronized InfGraph getPreload() {
		...
		if (cachePreload && preload == null && mode == HYBRID) {
			preload = new FBRuleInfGraph(this, rules, null, Factory.createDefaultGraph(), null);
			if (enableTGCCaching)
