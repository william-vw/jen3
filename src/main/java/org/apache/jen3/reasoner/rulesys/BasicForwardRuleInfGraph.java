...
import org.apache.jen3.reasoner.FGraph;
import org.apache.jen3.reasoner.Finder;
import org.apache.jen3.reasoner.FinderUtil;
...
/**
 * An inference graph interface that runs a set of forward chaining rules to
 * conclusion on each added triple and stores the entire result set.
 * <p>
 * This implementation has a horribly inefficient rule chainer built in. Once we
 * have this working generalize this to an interface than can call out to a rule
...
public class BasicForwardRuleInfGraph extends BaseInfGraph implements ForwardRuleInfGraphI {
	...
	/**
	 * The set of deduced triples, this is in addition to base triples in the fdata
	 * graph
	 */
	protected FGraph fdeductions;

	/**
	 * A safe wrapped version of the deductions graph used for reporting
	 * getDeductions
	 */
	...
	// (wvw) already kept by rule engine (minimize housekeeping)
	// private Set<Rule> rules;

	/**
	 * Flag, if true then find results will be filtered to remove functors and
	 * illegal RDF
	 */
...
// Core methods
	...
	/**
	 * Constructor. Creates a new inference graph to which a (compiled) rule set and
	 * a data graph can be attached. This separation of binding is useful to allow
	 * any configuration parameters (such as logging) to be set before the data is
	 * added. Note that until the data is added using {@link #rebind rebind} then
	...
	/**
	 * Constructor. Creates a new inference graph based on the given rule set. No
	 * data graph is attached at this stage. This is to allow any configuration
	 * parameters (such as logging) to be set before the data is added. Note that
	 * until the data is added using {@link #rebind rebind} then any operations like
	 * add, remove, find will result in errors.
	 * 
	 * @param reasoner the parent reasoner
	 * @param rules    the list of rules to use this time
	 * @param schema   the (optional) schema or preload data which is being
	 *                 processed
	 */
	...
	/**
	 * Constructor. Creates a new inference graph based on the given rule set then
	 * processes the initial data graph. No precomputed deductions are loaded.
	 * 
	 * @param reasoner the parent reasoner
	 * @param rules    the list of rules to use this time
	 * @param schema   the (optional) schema or preload data which is being
	...
	public BasicForwardRuleInfGraph(Reasoner reasoner, List<Rule> rules, Graph schema, Graph data,
			GraphConfig config) {
		this(reasoner, rules, schema, config);
		rebind(data);
	...
	/**
	 * Instantiate the forward rule engine to use. Subclasses can override this to
	 * switch to, say, a RETE imlementation.
	 * 
	 ...
	 * 
	 * @param ruleStore a compiled set of rules (i.e. the result of an
	 *                  FRuleEngine.compile).
	 */
	...
	public void addRule(Rule rule) {
		engine.addRule(rule, true);
		if (isPrepared())
			engine.fastInit(rule);
	}

	public boolean removeRule(Rule rule) {
		if (engine.removeRule(rule, true)) {
			rebind();
			return true;
	...
	public void rebind(boolean resetDeductions) {
		...
		this.resetDeductions = resetDeductions;
		this.setPreparedState(false);
	}

	@Override
	public void rebind(Node... predicates) {
		engine.deriveFor(predicates);
	}
	...
	public synchronized void prepare() {
		...
		if (fdeductions == null || resetDeductions) {
			// initialize the deductions graph
	...
	/**
	 * Adds a set of precomputed triples to the deductions store. These do not,
	 * themselves, fire any rules but provide additional axioms that might enable
	 * future rule firing when real data is added. Used to implement bindSchema
	 * processing in the parent Reasoner.
	 * 
	...
	public void addDeduction(Triple t) {
		...
		if (deducListener != null)
			deducListener.onDeduction(t);
	}
	...
	/**
	 * Extended find interface used in situations where the implementator may or may
	 * not be able to answer the complete query. It will attempt to answer the
	 * pattern but if its answers are not known to be complete then it will also
	 * pass the request on to the nested Finder to append more results.
	 * 
	 * @param pattern      a TriplePattern to be matched against the data
	 * @param continuation either a Finder or a normal Graph which will be asked for
	 *                     additional match results if the implementor may not have
	 *                     completely satisfied the query.
	 */
	@Override
	...
	/**
	 * Internals of findWithContinuation implementation which allows control over
	 * functor filtering.
	 */
	private ExtendedIterator<Triple> findWithContinuation(TriplePattern pattern,
			Finder continuation, boolean filter) {
		checkOpen();
		...
		// (wvw) unsure what the utility of this is ..
...
//        }
	...
	/**
	 * Returns an iterator over Triples. This implementation assumes that the
	 * underlying findWithContinuation will have also consulted the raw data.
	 */
	@Override
	...
	/**
	 * Basic pattern lookup interface. This implementation assumes that the
	 * underlying findWithContinuation will have also consulted the raw data.
	 * 
	 * @param pattern a TriplePattern to be matched against the data
	 * @return a ExtendedIterator over all Triples in the data set that match the
	 *         pattern
	 */
	@Override
...
//   the forward rule engine to invoke functions in this InfGraph
	...
	public void addBRule(Rule brule) {
		throw new ReasonerException(
				"Forward reasoner does not support hybrid rules - " + brule.toShortString());
	}
	...
	/**
	 * Return the Graph containing all the static deductions available so far. Will
	 * force a prepare.
	 */
	@Override
	...
	/**
	 * Create the graph used to hold the deductions. Can be overridden by subclasses
	 * that need special purpose graph implementations here. Assumes the graph
	 * underlying fdeductions and associated SafeGraph wrapper can be reused if
	 * present thus enabling preservation of listeners.
	 */
	protected Graph createDeductionsGraph() {
		...
		// (wvw) aha!
		safeDeductions = dg; // new SafeGraph( dg );
		return dg;
	}

	/**
	 * Return the Graph containing all the static deductions available so far. Does
	 * not trigger a prepare action. Returns a SafeWrapper and so can be used for
...
//        return fdeductions.getGraph();
	...
	/**
	 * Search the combination of data and deductions graphs for the given triple
	 * pattern. This may different from the normal find operation in the base of
...
// support for proof traces
	...
	/**
	 * Return the derivation of at triple. The derivation is a List of
	 * DerivationRecords
	 */
	@Override
	...
	/**
	 * Set the state of the trace flag. If set to true then rule firings are logged
	 * out to the Log at "INFO" level.
	 */
	...
	/**
	 * Return true if tracing should be acted on - i.e. if traceOn is true and we
	 * are past the bootstrap phase.
	 */
	@Override
