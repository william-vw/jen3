...
/**
 * An implementation of a binding environment that maintains a single array of
 * bound values for the variables in a rule. Stack management is done
 * externally. This is intended for use in the Brule system and so also supports
 * variable-variable bindings by use of reference chains.
 */
public class BindingVector implements BindingEnvironment {
	...
	/**
	 * Constructor - create a binding environment which is a copy of the given
	 * environment
	 */
	...
	/**
	 * Return the current array of bindings. Useful for fast access to several
	 * bindings, not useful for doing updates.
	 */
	 ...
	 */
	private Node getBinding(Node node) {
	 ...
	 * @return a TriplePattern obtained from the goal by substituting current
	 *         bindings
	 */
	public TriplePattern partInstantiate(TriplePattern goal) {
		return new TriplePattern(ground(goal.getSubject()), ground(goal.getPredicate()), ground(goal.getObject()));
	}
...
//    }
	...
	/**
	 * Unify a goal with the head of a rule. This is a poor-man's unification, we
	 * should try switching to a more conventional global-variables-with-trail
	 ...
	 * @return An initialized binding environment for the rule variables or null if
	 *         the unification fails. If a variable in the environment becomes
	 *         aliased to another variable through the unification this is
	 *         represented by having its value in the environment be the variable to
	 *         which it is aliased.
	 */
	...
	/**
	 * Unify a single pair of goal/head nodes. Unification of a head var to a goal
	 * var is recorded using an Integer in the head env to point to a goal env and
