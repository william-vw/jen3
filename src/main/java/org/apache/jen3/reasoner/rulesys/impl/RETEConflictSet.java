...
/**
 * Manages a set of ready-to-fire rules. For monotonic rule sets we simply fire
 * the rules as soon as they are triggered. For non-monotonic rule sets we stack
 * them up in a conflict set and fire them one-at-a-time, propagating all
...
public class RETEConflictSet {
	...
	/**
	 * Construct an empty conflict set, noting whether the overall rule system is
	 * monotonic or not
	 */
	...
	/**
	 * Record a request for a rule firing. For monotonic rulesets it may be actioned
	 * immediately, otherwise it will be stacked up.
	 */
	public void add(Rule rule, BindingEnvironment env, boolean isAdd) {
		if (isMonotonic) {
			RETERuleContext context = new RETERuleContext((ForwardRuleInfGraphI) gcontext.getGraph(),
					gcontext.getEngine());
			context.setEnv(env);
	...
	/**
	 * Pick on pending rule from the conflict set and fire it. Return true if there
	 * was a rule to fire.
	 */
	...
	public static void execute(RETERuleContext context, boolean isAdd) {
		...
		if (infGraph.shouldLogDerivations() && isAdd) {
			...
			for (int i = 0; i < rule.bodyLength(); i++) {
				...
				if (clause instanceof TriplePattern) {
					matchList.add(env.instantiateOutput((TriplePattern) clause, rule));
				}
			}
		}
		for (int i = 0; i < rule.headLength(); i++) {
			...
			if (hClause instanceof TriplePattern) {
				Triple t = env.instantiateOutput((TriplePattern) hClause, rule);
