...
import org.apache.jen3.graph.GraphEvents;
import org.apache.jen3.graph.Node;
import org.apache.jen3.graph.NodeFactory;
import org.apache.jen3.graph.TransactionHandler;
import org.apache.jen3.graph.Triple;
import org.apache.jen3.graph.compose.MultiUnion;
import org.apache.jen3.graph.impl.AllCapabilities;
...
import org.apache.jen3.reasoner.rulesys.BasicForwardRuleInfGraph;
...
public abstract class BaseInfGraph extends GraphBase implements InfGraph {
	...
	/**
	 * Flag to record if the preparation call has been made and so the graph is
	 * ready for queries
	 */
	...
	/**
	 * An optional DeductionListener instance for receiving new deductions.
	 * Currently this is only being set by {@link BasicForwardRuleInfGraph }
	 **/
	protected DeductionListener deducListener;

	protected boolean resetDeductions;

	/**
	 * Inference graphs share the prefix-mapping of their underlying raw graph.
	 * 
	 * @see org.apache.jen3.graph.Graph#getPrefixMapping()
	 */
	@Override
	 ...
	 *                 can be used to extract all entailments from the data.
	 */
	public BaseInfGraph(Graph data, Reasoner reasoner, DeductionListener listener) {
		super();
		this.fdata = new FGraph(data);
		this.reasoner = reasoner;
		this.deducListener = listener;
	}
	...
	/**
	 * An InfCapabilities notes that size may not be accurate, and some triples may
	 * be irremovable.
	 * 
...
//    	}
	...
	/**
	 * discard any state that depends on the content of fdata, because it's just
	 * been majorly trashed, solid gone.
	 */
	 ...
	 */
	@Override
	public synchronized void rebind(boolean resetDeductions) {
		version++;
	 ...
	 * 
	 * @return the deductions graph, if relevant for this class of inference engine
	 *         or null if not.
	 */
	@Override
	...
	/**
	 * Test a global boolean property of the graph. This might included properties
	 * like consistency, OWLSyntacticValidity etc. It remains to be seen what level
	 * of generality is needed here. We could replace this by a small number of
	 * specific tests for common concepts.
	 * 
	 * @param property the URI of the property to be tested
	 * @return a Node giving the value of the global property, this may be a boolean
	 *         literal, some other literal value (e.g. a size).
	 */
	@Override
	...
	/**
	 * A convenience version of getGlobalProperty which can only return a boolean
	 * result.
	 */
	@Override
	...
	/**
	 * Test the consistency of the bound data. This normally tests the validity of
	 * the bound instance data against the bound schema data.
	 * 
	...
	/**
	 * An extension of the Graph.find interface which allows the caller to encode
	 * complex expressions in RDF and then refer to those expressions within the
	 * query triple. For example, one might encode a class expression and then ask
	 * if there are any instances of this class expression in the InfGraph.
	 * 
	 ...
	 *
	 * <p>
	 * This code used to have the .filterKeep component uncommented. We think this
	 * is because of earlier history, before .matches on a literal node was
	 * implemented as sameValueAs rather than equals. If it turns out that the
	 * filter is needed, it can be commented back in, AND a corresponding filter
	 * added to find(Node x 3) -- and test cases, of course.
	 */
	@Override
	protected ExtendedIterator<Triple> graphBaseFind(Triple triple) {
		return graphBaseFind(triple.getMatchSubject(), triple.getMatchPredicate(),
	 ...
	 */
	abstract public ExtendedIterator<Triple> findWithContinuation(TriplePattern pattern,
			Finder continuation);
	...
	/**
	 * Answer true iff this graph is empty. [Used to be in QueryHandler, but moved
	 * in here because it's a more primitive operation.]
	 */
	@Override
	...
	/**
	 * Return a version stamp for this graph which can be used to fast-fail
	...
	/**
	 * Return a new inference graph which is a clone of the current graph together
	 * with an additional set of data premises. The default implementation loses ALL
	 * partial deductions so far. Some subclasses may be able to a more efficient
	 * job.
	 */
	 ...
	 */
	public synchronized void setPreparedState(boolean state) {
		this.isPrepared = state;
	}

	/**
	 * Checks whether the graph is prepared and calls {@link #prepare()} if it is
	 * not
	 */
	...
	public Node uniqueBlankNode(Scope scope) {
		return NodeFactory.createBlankNode(scope);
	}
}
