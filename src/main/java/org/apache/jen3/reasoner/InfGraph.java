 ...
 * graph. Many entailments from the raw data are made to appear as if they are
 * extract triples in the inferred graph and so appear through the normal
 * Graph.find interface.
 * 
 * However, here are two extensions required. Firstly, the ability to ask about
 * global properties of the whole graph (e.g. consistency). Secondly, the
 * ability to temporarily construct expressions (encoded in RDF) which form more
 * complex queries.
 */
public interface InfGraph extends Graph {
	...
	/**
	 * Cause the inference graph to derive inferences only for the given predicates.
	 * This requires the N3ModelSpec to have been setup with these predicates
	 * (allowRederiveFor) so rules with these predicates can be indexed.
	 * 
	 * @param predicates
	 */
	...
	/**
	 * Sets the deduction listener for this graph.
	 * 
	 * @param listener
	 */
	 ...
	 * previous find operation). Not all reasoners will support derivations.
	 * 
	...
	/**
	 * Coins a unique blank node within this graph.
	 * 
	 * @return
	 */
	...
	/**
	 * Listener for deductions in a forward reasoner.
	 * 
	 * @author wvw
	 *
	 */
	public interface DeductionListener {
