...
/**
 * Datastructure which defines a triple pattern as used in simple rules and in
 * find interfaces.
 * <p>
 * Wildcards are recorded by using Node_RuleVariable entries rather than nulls
 * because they can be named. If a null is specified that is converted to a
 * variable of name "". Note that whilst some engines might simply require
 * Node_Variables the forward engine requires variables represented using the
 * more specialized subclass - Node_RuleVariable.
 * </p>
 * <p>
 * It would make more sense to have TriplePattern subclass Triple but that is
...
public class TriplePattern implements ClauseEntry {
	...
	// not utilized in equality-related methods;
	// merely a flag for knowing which triples (not) to retract
	protected boolean stableTruth;
	...
	/**
	 * Constructor - builds a pattern from three nodes, use Node_RuleVariables as
	 * variables, use a variable with an empty name as a wildcard, can also use null
	 * as a wildcard.
	 */
	...
	public TriplePattern(Node subject, Node predicate, Node object, boolean stableTruth) {
		this(subject, predicate, object);

		this.stableTruth = stableTruth;
	}

	/**
	 * Constructor - builds a degenerate pattern from a simple triple. This would be
	 * much easier if we merged Triples and TriplePatterns!
	 */
	...
	public boolean isStableTruth() {
		return stableTruth;
	}

	public void setStableTruth(boolean stableTruth) {
		this.stableTruth = stableTruth;
	}

	public Node get(byte position) {
		switch (position) {
		case Triple.S:
			return subject;
		case Triple.P:
	...
	/**
	 * Compare two patterns for compatibility - i.e. potentially unifiable. Two
	 * patterns are "compatible" in the sense we mean here if all their ground terms
	 * match. A variable in either pattern can match a ground term or a variable in
	 * the other. We are not, currently, checking for multiple occurances of the
	 * same variable. Functor-valued object literals are treated as a special case
	 * which are only checked for name/arity matching.
	 */
	...
	/**
	 * Test if a pattern is just a variant of this pattern. I.e. it is the same up
	 * to variable renaming. This takes into account multiple occurances of the same
	 * variable.
	 */
	...
	/**
	 * Check a pattern to see if it is legal, used to exclude backchaining goals
	 * that could never be satisfied. A legal pattern cannot have literals in the
	 * subject or predicate positions and is not allowed nested functors in the
	 * object.
	 */
	...
	/**
	 * Compare two patterns and return true if arg is a more specific (more
	 * grounded) version of this one. Does not handle functors.
	 */
	...
	/**
	 * Convert any Node_RuleVariable wildcards to null. This loses the variable
	 * named but is used when converting a singleton pattern to a TripleMtch
	 */
	...
	/**
	 * Equality override - used so that TriplePattern variants (same to within
	 * variable renaming) test as equals
	 */
	@Override
...
//                object.equals(((TriplePattern)o).object);
		return o instanceof TriplePattern && nodeEqual(subject, ((TriplePattern) o).subject)
				&& nodeEqual(predicate, ((TriplePattern) o).predicate) && nodeEqual(object, ((TriplePattern) o).object);
	}
...
//        return (subject.hashCode() >> 1) ^ predicate.hashCode() ^ (object.hashCode() << 1);
	...
	/**
	 * Compare triple patterns, taking into account variable indices. The equality
	...
	public boolean sameAs(Object o) {
		...
		return Node_RuleVariable.sameNodeAs(subject, other.subject)
				&& Node_RuleVariable.sameNodeAs(predicate, other.predicate)
