...
/**
 * The interface to be satisfied by implementations maintaining collections of
 * RDF triples. The core interface is small (add, delete, find, contains) and is
 * augmented by additional classes to handle more complicated matters such as
 * event management.
 * 
 * @see GraphBase for an implementation framework.
 */
public interface Graph extends ScopedObject {
	/**
	...
	/**
	 * true if this graph's content depends on the other graph. May be pessimistic
	 * (ie return true if it's not sure). Typically true when a graph is a
	...
	/**
	 * returns this Graph's prefix mapping. Each call on a given Graph gets the same
	 * PrefixMapping object, which is the one used by the Graph.
	 */
	...
	/**
	 * Returns an iterator over all Triples in the graph. Equivalent to
	 * {@code find(Node.ANY, Node.ANY, Node.ANY)}
	 * 
	 * @return an iterator of all triples in this graph
	 */
	...
	/**
	 * Compare this graph with another using the method described in
	...
	/**
	 * Answer true iff the graph contains a triple matching (s, p, o). s/p/o may be
	 * concrete or fluid. Equivalent to find(s,p,o).hasNext, but an implementation
	 ...
	 * can manage", because an inference graph may have irremovable axioms and their
	 * consequences.
	 */
	boolean isEmpty();
	 ...
	 * bound of the number of triples. For example, an inference graph might return
	 * the number of triples in the raw data graph.
	 */
	int size();
	...
	N3GraphConfig getN3Config();

	GraphStats getStats();

	public static interface GraphConfig {
	}

	public static class GraphStats {

		private boolean includesVars;
		private List<Node_RuleVariable> ruleVars = new ArrayList<>();

		public GraphStats() {
		}

		public GraphStats(boolean includesVars) {
			this.includesVars = includesVars;
		}
		...
		public void includesVar(Node var) {
			this.includesVars = true;
		}

		public boolean includesRuleVars() {
			return !ruleVars.isEmpty();
		}

		public void includesRuleVar(Node node) {
			if (node.isRuleVariable())
				ruleVars.add((Node_RuleVariable) node);
			else
				ruleVars.addAll(((Node_Compound) node).getRuleVars());
		}
		...
		public String toString() {
			return (includesRuleVars() ? "{vars=" + ruleVars + "}"
					: (includesVars ? "{vars}" : ""));
		}
	}
}
