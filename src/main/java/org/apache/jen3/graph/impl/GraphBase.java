 ...
 * finds (on top of that one), a simple-minded prepare, and contains. GraphBase
 * also handles the event-listening and registration interfaces.
 * <p>
...
public abstract class GraphBase implements GraphWithPerform {
	...
	protected GraphStats stats = new GraphStats();
	 ...
	 * supplied to the reifier when it is created).
	 */
	...
	/**
	 * Default implementation answers <code>true</code> iff this graph is the same
	 * graph as the argument graph.
	 */
	@Override
	 ...
	 * object (the same one each time, not that it matters - Capabilities should be
	 * immutable).
	 */
	@Override
	...
	private void checkAdd(Node n) {
		...
		// only a problem when creating models programmatically

		if (n.isVariable() || n.includesVars())
			stats.includesVar(n);

		if (n.isRuleVariable() || n.includesRuleVars())
	...
	*/
	 ...
	 * fine if <code>size</code> is reasonable efficient. Subclasses may override if
	 * necessary. This method may become final and defined in terms of other
	 * methods.
	 */
	@Override
	...
	// EnhGraph relies on this hashcode
	 ...
	 * <code>prefix</code> will appear near the beginning of the string. Nodes may
	 * be prefix-compressed using <code>that</code>'s prefix-mapping. This default
