...
import org.apache.jen3.datatypes.xsd.impl.XMLLiteralType;
...
/**
 * Represents the "contents" of a Node_Literal. These contents comprise a
 * lexical form, an optional language tag, and optional datatype structure and a
 * value.
 */
final /* public */ class LiteralLabelImpl implements LiteralLabel {
	...
	/**
	 * The lexical form of the literal, may be null if the literal was created
	 * programatically and has not yet been serialized
	 */
	private String lexicalForm;

	/**
	 * The value form of the literal. It will be null only if the value has not been
	 * parsed or if it is an illegal value. For plain literals and xsd:string
	 * literals the value is the same as the lexicalForm.
	 */
	private Object value;

	/**
	 * The type of the literal. A null type indicates a classic "plain" literal. The
	 * type of a literal is fixed when it is created.
	 */
	private RDFDatatype dtype;

	/**
	 * The xml:lang tag. For xsd literals this is ignored and not part of equality.
	 * For plain literals it is not ignored. The lang of a literal is fixed when it
	 * is created.
	 */
	 ...
	 * ill-formed literals to be treated as syntactically correct so instead of only
	 * storing well-formed literals we hack around it this way. N.B. This applies to
	 * any literal, not just XML well-formed literals.
	 */
	private boolean wellformed = true;

	/**
	 * keeps the message provided by the DatatypeFormatException if parsing failed
	 * for delayed exception thrown in getValue()
	 */
	...
	/**
	 * Build a typed literal label from its lexical form. The lexical form will be
	...
	private void setLiteralLabel_1(String lex, String lang, RDFDatatype dtype) throws DatatypeFormatException {
		...
		this.dtype = dtype;
		this.lang = (lang == null ? "" : lang.startsWith("@") ? lang.substring(1) : lang);
		if (dtype == null) {
			value = lex;
		} else {
			setValue(lex);
	 ...
	 * currently registered as the default representation for this java class. No
	...
	private void setLiteralLabel_2(Object value, String language, RDFDatatype dtype) throws DatatypeFormatException {
		...
		if (dtype != null && lexicalForm == null) {
			// We are creating a literal from a java object, check the lexical form of the
	 ...
	 * 
	 * @param xml If true then s is exclusive canonical XML of type rdf:XMLLiteral,
	 *            and no checking will be invoked.
	 * 
	 */
	...
	/**
	 * Internal function to set the object value from the lexical form. Requires
	 * datatype to be set.
	 * 
	 * @throws DatatypeFormatException if the value is ill-formed and eager checking
	 *                                 is on.
	 */
	 ...
	 * (e.g. value is xsd:date but the time is xsd:datetime) it will narrow the
	 * type. If the type is narrower than the value then it may normalize the value
	 * (e.g. set the mask of an XSDDateTime)
	 */
	...
	/**
	 * Answer a human-acceptable representation of this literal value. This is NOT
	 * intended for a machine-processed result.
	 */
	@Override
	...
	/**
	 * Answer the value used to index this literal TODO Consider pushing indexing
	 * decisions down to the datatype
	 */
	@Override
	public Object getIndexingValue() {
		return isXML() ? this
				: !lang.equals("") ? getLexicalForm() + "@" + lang.toLowerCase(Locale.ROOT)
						: wellformed ? getValue() : getLexicalForm();
	}
	...
	public boolean equals(Object other) {
		...
		if (dtype == null ^ otherLiteral.getDatatype() == null)
			return false;

		if (dtype != null) {
			boolean typeEquals = dtype.getURI().equals(otherLiteral.getDatatype().getURI());
			if (!typeEquals)
				return false;
		}

		// Don't just use this.lexcialForm -- need to force delayed calculation from
		// values.
	...
	/**
	 * Answer true iff this literal represents the same (abstract) value as the
	 * other one.
	 */
	@Override
	...
	/**
	 * Two literal labels are the "same value" if they are the same string, or same
	 * language string or same value-by-datatype or .equals (= Same RDF Term)
	 * 
	 * @param lit1
	 * @param lit2
	 * @return
