...
import org.apache.jen3.graph.impl.GraphWithPerform;
import org.apache.jen3.util.IteratorCollection;
...
public class GraphUtil {
	...
	/**
	 * Return an iterator over the unique subjects with predicate p and object o. p
	 * and o can be wildcards (Node.ANY)
	 * 
	 * @param g Graph
	...
	/**
	 * Return an *ordered* iterator over non-unique subjects with predicate p and
	 * object o. p and o can be wildcards (Node.ANY). E.g., needed when printing
	 * cited formulas.
	 * 
	 * @param g Graph
	 * @param p Predicate - may be Node.ANY
	 ...
	 * @return ExtendedIterator
	 */
	public static ExtendedIterator<Node> listOrderedSubjects(Graph g, Node p, Node o) {
		ExtendedIterator<Triple> iter = g.find(Node.ANY, p, o);
		return WrappedIterator.createNoRemove(iter.mapWith(t -> t.getSubject()));
	}

	/**
	 * Return an iterator over the unique predicate between s and o. s and o can be
	 * wildcards (Node.ANY)
	 * 
	 * @param g Graph
	...
	/**
	 * Return an iterator over the unique objects with a given subject and object. s
	 * and p can be wildcards (Node.ANY)
	 * 
	 * @param g Graph
	 * @param s Subject - may be Node.ANY
	 * @param p Predicate - may be Node.ANY
	...
	/**
	 * A safe and cautious remove() function that converts the remove to a number of
	 * {@link Graph#delete(Triple)} operations.
	 * <p>
	 * To avoid any possible ConcurrentModificationExceptions, it finds batches of
	 * triples, deletes them and tries again until no more triples matching the
	 * input can be found.
	 */
	...
	/**
	 * Delete triples in {@code srcGraph} from {@code dstGraph} by looping on
	 * {@code srcGraph}.
	 * 
	 */
	...
	/**
	 * Delete the triple in {@code srcGraph} from {@code dstGraph} by checking the
	 * contents of {@code dsgGraph} against the {@code srcGraph}. This involves
	 * calling {@code srcGraph.contains}.
	 * 
	 * @implNote {@code dstGraph.size()} is used by this method.
	 */
	 ...
	 * modification" safe; it assumes it can use the iterator while deleting from
	 * the graph.
	 */
	 ...
	 *
	 * @implNote This is designed for the case of {@code dstGraph} being comparable
	 *           or much larger than {@code srcGraph} or {@code srcGraph} having a
	 *           lot of triples to actually be deleted from {@code dstGraph}. This
	 *           includes teh case of large, persistent {@code dstGraph}.
	 *           <p>
	 *           It is not designed for a large {@code srcGraph} and large
	 *           {@code dstGraph} with only a few triples in common to delete from
	 *           {@code dstGraph}. It is better to calculate the difference in some
	 *           way, and copy into a small graph to use as the {@code srcGraph}.
	 *           <p>
	 *           To force delete by looping on {@code srcGraph}, use
	 *           {@link #deleteLoopSrc(Graph, Graph)}.
	 *           <p>
	 *           For large {@code srcGraph} and small {@code dstGraph}, use
	 *           {@link #deleteLoopDst}.
	 * 
	 *           See discussion on <a href=
	 *           "https://github.com/apache/jena/pull/212">jena/pull/212</a>,
	 *           (archived at <a href=
	 *           "https://issues.apache.org/jira/browse/JENA-1284">JENA-1284</a>).
	 */
	public static void deleteFrom(Graph dstGraph, Graph srcGraph) {
		...
		// Loop on dstGraph, not srcGraph, but need to use srcGraph.contains on this
		// code path.
	...
	private static boolean decideHowtoExecuteBySizeSize(Graph dstGraph, Graph srcGraph) {
		// Loop on src if:
		// size(src) <= MIN_SRC_SIZE : srcGraph is below the threshold MIN_SRC_SIZE (a
		// "Just Do it" number)
	...
	/**
	 * Avoid dstGraph.size(). Instead step through {@codedstGraph.find} to compare
	 * to {@code srcGraph.size()}
	 * 
	 */
	...
	/**
	 * Compare the size of a graph to {@code size}, without calling Graph.size by
	 * iterating on {@code graph.find()} as necessary.
	 * <p>
