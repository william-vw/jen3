...
import org.apache.jen3.n3.io.JenaURIException;
import org.apache.jen3.n3.io.N3IRIResolver;
...
// NOTE public methods to support composite (vs. inheritance) pattern
...
public class ParserBase {
	...
	public Node createLiteralInteger(String lexicalForm) {
		return NodeFactory.createLiteral(lexicalForm, null, XSDDatatype.XSDinteger);
	}

	public Node createLiteralDouble(String lexicalForm) {
		return NodeFactory.createLiteral(lexicalForm, null, XSDDatatype.XSDdouble);
	}

	public Node createLiteralDecimal(String lexicalForm) {
		return NodeFactory.createLiteral(lexicalForm, null, XSDDatatype.XSDdecimal);
	}

	public Node createLiteral(String lexicalForm, String langTag, Node datatype) {
		String uri = (datatype == null) ? null : datatype.getURI();
		return createLiteral(lexicalForm, langTag, uri);
	}

	public Node createLiteral(String lexicalForm, String langTag, String datatypeURI) {
		RDFDatatype dType = null;
		if (datatypeURI != null)
			dType = TypeMapper.getInstance().getSafeTypeByName(datatypeURI);
		
		return NodeFactory.createLiteral(lexicalForm, langTag, dType);
	}

	public Node createLiteral(String lexicalForm, String langTag, RDFDatatype datatype) {
		return NodeFactory.createLiteral(lexicalForm, langTag, datatype);
	}

	public long integerValue(String s) {
		if (s.startsWith("+"))
			s = s.substring(1);
		if (s.startsWith("0x")) {
			// Hex
	...
	public String stripQuotes3(String s) {
		return s.substring(3, s.length() - 3);
	}

	public String stripChars(String s, int n) {
		return s.substring(n, s.length());
	}

	public String resolveQuotedIRI(String iriStr, int line, int column) {
		iriStr = stripQuotes(iriStr);
		return resolveIRI(iriStr, line, column);
	}

	public String resolveIRI(String iriStr, int line, int column) {
	...
	public String resolvePName(String qname, int line, int column) {
	...
	public Node createBNode() {
		return bNodeLabels.allocNode(null);
	}

	// Labelled bNode.
	public Node createBNode(String label, int line, int column) {
		return bNodeLabels.asNode(label, null);
	}

	public Node createBNode(String label) {
		return bNodeLabels.asNode(label, null);
	}

	public Node createVariable(String s, int line, int column) {
		s = s.substring(1); // Drop the marker
		return NodeFactory.createVariable(s);
	}

	public Node createNode(String iri) {
	...
	public boolean isBNodeIRI(String iri) {
		return skolomizedBNodes && iri.startsWith(bNodeLabelStart);
	}

//    public  Node createNodeFromURI(String s, int line, int column)
//    {
//        s = stripQuotes(s) ;
...
//    }
	...
	public void throwParseException(String s, int line, int column) {
		throw new TurtleParseException(exMsg(s, line, column));
	}

	public String fixupPrefix(String prefix, int line, int column) {
		if (prefix.endsWith(":"))
...
//
//    public  String unescapeCodePoint(String s, int line, int column)
//    { return unescape(s, '\\', true, line, column) ; }

	public String unescapeStr(String s, int line, int column) {
...
//        }
	...
	public static String exMsg(String msg, int line, int column) {
