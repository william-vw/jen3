...
/**
 * An N3 pretty printer. Tries to make N3 data look readable - works better on
 * regular data.
 */
...
/* implements RDFWriter */
{
	...
	protected Set<RDFObject> oneRefObjects = null; // Bnodes referred to once as an object - can inline
	...
	public N3JenaWriterPP(BNodeWriter writer) {
		super(writer);
	}
...
//        }
	...
	protected void prepareOneRefBNodes(Model model) {
		...
		ResIterator objIter = model.listObjects();
		for (; objIter.hasNext();) {
			Resource n = objIter.nextResource();
	...
	protected boolean testOneRefBNode(RDFObject n) {
	...
	protected ClosableIterator<Resource> preparePropertiesForSubject(Resource r) {
		...
		SortedMap<String, Resource> tmp1 = new TreeMap<>();
		SortedMap<String, Resource> tmp2 = new TreeMap<>();

		StmtIterator sIter = r.listProperties();
		for (; sIter.hasNext();) {
			Resource p = sIter.nextStatement().getPredicate();
			if (seen.contains(p))
				continue;
			seen.add(p);
		...
		ExtendedIterator<Resource> eIter = null;

		if (hasTypes)
			eIter = new SingletonIterator<>(RDF.type);

		ExtendedIterator<Resource> eIter2 = WrappedIterator.create(tmp1.values().iterator());
...
//            out.println(" .");
		...
		for (RDFObject oneRefObject : oneRefObjects) {
			out.println();
			if (N3JenaWriter.DEBUG) {
				out.println("# One ref");
			}
	...
	// Very hard to do a pretty thing here because the objects may be large or small
	// or a mix.

	@Override
	protected void writeObjectList(Resource subject, Resource property) {
...
//        }
		...
		Set<Resource> simple = new HashSet<>();
		Set<Resource> complex = new HashSet<>();

		for (; sIter.hasNext();) {
			Statement stmt = sIter.nextStatement();
			Resource obj = stmt.getObject();
		...
		if (simple.size() > 0) {
			...
			if (doObjectListsAsLists) {
				...
				for (Iterator<Resource> iter = simple.iterator(); iter.hasNext();) {
		...
		if (complex.size() > 0) {
			...
			for (Iterator<Resource> iter = complex.iterator(); iter.hasNext();) {
	...
	protected boolean isSimpleObject(RDFObject node) {
	...
	protected void writeObject(Resource node) {
		...
		if (allowDeep && !isSimpleObject(rObj)) {
			oneRefDone.add(rObj);
			ClosableIterator<Resource> iter = preparePropertiesForSubject(rObj);
			if (!iter.hasNext()) {
				// No properties.
				out.print("[]");
			} else {
	...
	protected void writeList(Resource resource) {
		...
		for (Iterator<Resource> iter = rdfListIterator(resource); iter.hasNext();) {
			if (!listFirst)
				out.print(" ");
			listFirst = false;
			Resource n = iter.next();
	...
	protected void writeListUnpretty(Resource r) {
		// for ( ; ! r.equals(RDF.nil); )
		{
			...
			StmtIterator sIter = r.getModel().listStatements(r, null, (Resource) null);
			for (; sIter.hasNext();) {
			...
			for (; sIter.hasNext();) {
				Statement s = sIter.next();
				RDFObject nextNode = s.getObject();
	...
	private void writeStatement(Statement s) {
		...
		out.print(formatResource(s.getPredicateProperty()));
		out.print(" ");
	...
	protected N3JenaWriterCommon copy() {
		return new N3JenaWriterPP(bnodeWriter);
	}
}
