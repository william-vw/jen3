...
/**
 * A simple N3 writer - writes N3 out as triples with prefixes done. "N3
 * triples" - triples with N3 abbreviations and prefixes. Very simple.
 */

public class N3JenaWriterTriples extends N3JenaWriterCommon {
	static public final int colWidth = 8;

	public N3JenaWriterTriples() {
	}

	public N3JenaWriterTriples(BNodeWriter writer) {
		super(writer);
	}

	@Override
	protected void writeModel(Model model, boolean nested) {
		...
		for (; sIter.hasNext();) {
			...
			String predStr = formatProperty(stmt.getPredicateProperty());
			out.print(predStr);
			padCol(predStr);
			out.print(minGapStr);
	...
	protected N3JenaWriterCommon copy() {
		return new N3JenaWriterTriples(bnodeWriter);
	}
}
