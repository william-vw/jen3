...
public abstract class GraphTripleStoreBase implements TripleStore {
	...
	protected GraphTripleStoreBase(Graph parent, NodeToTriplesMapBase subjects, NodeToTriplesMapBase predicates,
	...
	public ExtendedIterator<Triple> find(Triple t) {
		boolean matchAbsolute = t.getSubject().doMatchAbsolute() || t.getPredicate().doMatchAbsolute()
				|| t.getObject().doMatchAbsolute();

		Node s = t.getSubject();
		Node o = t.getObject();
		Node p = t.getPredicate();

		byte position = Selectivity.chooseIndex(t, matchAbsolute);
		switch (position) {

		case Triple.S:
			return new StoreTripleIterator(parent, subjects.iterator(s, p, o, matchAbsolute), subjects, predicates,
					objects);

		case Triple.P:
			return new StoreTripleIterator(parent, predicates.iterator(p, o, s, matchAbsolute), predicates, subjects,
					objects);

		case Triple.O:
			return new StoreTripleIterator(parent, objects.iterator(o, s, p, matchAbsolute), objects, subjects,
					predicates);
