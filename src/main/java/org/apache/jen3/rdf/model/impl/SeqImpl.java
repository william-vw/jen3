...
public class SeqImpl extends ContainerImpl implements Seq {
    ...
    @Override
    public RDFObject getObject(int index)  {
    ...
    @Override
    public Seq set(int index, RDFObject o)  {
        checkIndex(index);
    ...
    public Seq set(int index, Object o)  {
        ...
        Model m = s.getModel() ;
        Statement s2 = m.createLiteralStatement(s.getSubject(), s.getPredicateProperty(), o) ; 
        s.getModel().remove(s) ;
        s.getModel().add(s2) ;
        //getRequiredProperty(RDF.li(index)).changeObject(o);
        return this;
    ...
    @Override
    public Seq add(int index, Resource o)  {
        int size = size();
    ...
    public Container remove(Statement s) {
        ...
        shiftDown(s.getPredicateProperty().getOrdinal()+1, size()+1);
