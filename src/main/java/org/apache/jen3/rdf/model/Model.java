...
import org.apache.jen3.shared.Lock;
 ...
 * resources, properties and literals and the Statements which link them, for
 * adding statements to and removing them from a model, for querying a model and
 * set operations for combining models.
 * <p>
 * Models may create Resources [URI nodes and bnodes]. Creating a Resource does
 * <i>not</i> make the Resource visible to the model; Resources are only "in"
 * Models if Statements about them are added to the Model. Similarly the only
 * way to "remove" a Resource from a Model is to remove all the Statements that
 * mention it.
 * <p>
 * When a Resource or Literal is created by a Model, the Model is free to re-use
 * an existing Resource or Literal object with the correct values, or it may
 * create a fresh one. [All Jena RDFNodes and Statements are immutable, so this
 * is generally safe.]
 * <p>
 * This interface defines a set of primitive methods. A set of convenience
 * methods which extends this interface, e.g. performing automatic type
 * conversions and support for enhanced resources, is defined in
 * {@link ModelCon}.
 * </P>
 * 
 ...
 * 
 * Some of the methods, e.g. the read methods, may have to traverse a firewall.
 * This can be accomplished using the standard java method of setting system
 * properties. To use a socks proxy, include on the java command line:
 * </p>
 * <blockquote> -DsocksProxyHost=[your-proxy-domain-name-or-ip-address]
 * </blockquote>
 *
 * <p>
 * To use an http proxy, include on the command line:
 * </p>
 * <blockquote> -DproxySet=true -DproxyHost=[your-proxy]
 * -DproxyPort=[your-proxy-port-number] </blockquote>
 *
 * <p>
...
public interface Model extends ModelCon, ModelGraphInterface, RDFReaderF, RDFWriterF, PrefixMapping, Lock {
	 ...
	 * 
	 * @return the number of statements in a concrete model or an estimated lower
	 *         bound on the number of statements in an virtualized model
	 */
	long size();

	/**
	 * Answer true iff the model contains no explicit statements (ie it's size is
	 * zero, listStatements() would deliver the empty iterator).
	 * 
	...
	/**
	 * (You probably don't want this method; more likely you want the PrefixMapping
	 * methods that Model supports.) List the namespaces used by predicates and
	 * types in the model. This method is really intended for use by the RDF/XML
	 * writer, which needs to know these namespaces to generate correct and vaguely
	 * pretty XML.
	 * <p>
	 * The namespaces returned are those of (a) every URI used as a property in the
	 * model and (b) those of every URI that appears as the object of an rdf:type
	 * statement.
	 * <p>
	 * Note that the notion of "namespace" used here is not that of an XML
	 * prefix-namespace, but just of the minimal legal left part of a URI (see
	 * Util.splitNamespace for details). If you want the RDF/XML (or N3) namespaces,
	 * treat the Model as a PrefixMapping.
	 * 
	 * @see org.apache.jen3.shared.PrefixMapping
	 ...
	 * legacy: createResource is now capable of, and allowed to, reuse existing
	 * objects.
	 * <p>
	 ...
	 * have a new AnonId distinct from any allocated by any other call of this
	 * method.
	 * <p>
	 ...
	 * <p>
	 * This method may return an existing bnode with the correct AnonId and model,
	...
	/**
	 * Create a new resource associated with this model. If the uri string is null,
	 * this creates a bnode, as per <code>createResource()</code>. Otherwise it
	 * creates a URI node. A URI resource is .equals() to any other URI Resource
	 * with the same URI (even in a different model - be warned).
	 * <p>
	...
	/**
	 * Build a typed literal node from its lexical form. The lexical form will be
	 * parsed now and the value stored. If the form is not legal this will throw an
	 * exception.
	 * 
	 ...
	 */	
	public Literal createLiteral(String lex, RDFDatatype dtype) throws DatatypeFormatException;
	
	/**
	 * Create a literal from a String value. An existing literal of the right value
	 * may be returned, or a fresh one created. The use of the wellFormed flag is to
	 * create typed literals of type rdf:XMLLiteral, without error checking. This
	 * should only be use when the lexical form is known to already be in exclusive
	 * canonical XML.
	 * 
	 * @param v          the lexical form of the literal
	 * @param wellFormed true if the Literal is well formed XML, in the lexical
	...
	/**
	 * Build a typed literal from its lexical form. The lexical form will be parsed
	 * now and the value stored. If the form is not legal this will throw an
	 * exception.
	 * <p>
	 * Note that in preview releases of Jena2 it was also possible to specify a
	 * language type. Changes to the RDF specification mean that this is no longer
	 * legal except for plain literals. To create a plain literal with a language
	 * tag use {@link #createLiteral(String, String) createLiteral}.
	 * 
	 * @param lex   the lexical form of the literal
	...
	/**
	 * Build a typed literal label from its value form using whatever datatype is
	 * currently registered as the the default representation for this java class.
	 * No language tag is supplied.
	 * 
	...
	/**
	 * Create a Statement instance. (Creating a statement does not add it to the set
	 * of statements in the model; see Model::add). This method may return an
	 * existing Statement with the correct components and model, or it may construct
	 * a fresh one, as it sees fit.
	 * <p>
	 * Subsequent operations on the statement or any of its parts may modify this
	 * model.
	 * 
	 ...
	 */
	public Statement createStatement(Resource s, Resource p, RDFObject o);
	 ...
	 * model.
	 * 
	 * @param t the triple that will be wrapped by the statement
	 * @return the new statement
	 */
	public Statement createStatement(Triple t);
	
	/**
	 * Answer a new empty list. This is equivalent to a list consisting only of
	 * <code>rdf:nil</code>.
	 * 
	 ...
	 */
	public RDFList createList(Iterator<? extends Resource> members);

	/**
	 * <p>
	 * Answer a new list containing the nodes from the given eleemnts, in order. If
	 * the list of elements is empty, until the list is made the object or subject
	 * in the model, or has an element added, it will not appear in the model (e.g.
	 * when written out).
	 *
	 * @param members An array of RDF nodes that will be the members of the list
	 * @return An RDF-encoded list
	 */
	public RDFList createList(Resource... members);
	...
	/**
	 * Remove all the statements in the list from this model, using the bulk update
	 * interface.
	 * 
	...
	/**
	 * Add all the statements in another model to this model, including the reified
	 * statements.
	 * 
	 * @return this model
	...
	/**
	 * Add the RDF statements from a document. Uses content negotiation to request
	 * appropriate mime types. If the content type is not found, it may guess from
	 * the URL.
	 * <p>
	 ...
	 * Writing RDF in Apache Jena"</a> for more information about determining the
	 * syntax.
	 * </p>
	 *
	 * @return this model
	 ...
	 * RDF/XML. See {@link #read(InputStream, String, String)} for explicitly
	 ...
	 * (or "TTL") and "N3". <code>null</code> represents the default language,
	 ...
	 *             <p>
	 *             See <a href=
	 *             "http://jena.apache.org/documentation/io/index.html">"Reading and
	 *             Writing RDF in Apache Jena"</a> for more information about
	 *             concrete syntaxes.
	 *             </p>
	 * 
	...
	/**
	 * Using this method is often a mistake. Add statements from an RDF/XML
	 ...
	 * 
	 * It is better to explicitly set the serialization format. See
	 * {@link #read(InputStream, String, String)} for explicitily setting the
	...
	/**
	 * Add statements from a serializion in language <code>lang</code> to the model.
	 * <br />
	 * Predefined values for <code>lang</code> are "RDF/XML", "N-TRIPLE", "TURTLE"
	 * (or "TTL") and "N3". <code>null</code> represents the default language,
	 * "RDF/XML". "RDF/XML-ABBREV" is a synonym for "RDF/XML". <br />
	...
	/**
	 * Using this method is often a mistake. Add RDF statements represented in
	 * language <code>lang</code> to the model. <br />
	 * Predefined values for <code>lang</code> are "RDF/XML", "N-TRIPLE", "TURTLE"
	 * (or "TTL") and "N3". <code>null</code> represents the default language,
	 * "RDF/XML". "RDF/XML-ABBREV" is a synonym for "RDF/XML". <br />
	 ...
	 * 
	 * @param base   the base uri to be used when converting relative URI's to
	 *               absolute URI's. (Resolving relative URIs and fragment IDs is
	 *               done by prepending the base URI to the relative URI/fragment.)
	 *               If there are no relative URIs in the source, this argument may
	 *               safely be <code>null</code>. If the base is the empty string,
	 *               then relative URIs <i>will be retained in the model</i>. This
	 *               is typically unwise and will usually generate errors when
	...
	/**
	 * Read into this model the RDF at <code>url</code>, using <code>baseURI</code>
	 * as the base URI if it is non-null. The RDF is assumed to be RDF/XML unless
	 * <code>lang</code> is non-null, in which case it names the language to be
	 * used. Answer this model.
	 */
	...
	/**
	 * <p>
	 * Write the model as an XML document. It is often better to use an OutputStream
	...
	/**
	 * <p>
	 * Write a serialized representation of a model in a specified language. It is
	 * often better to use an OutputStream rather than a Writer, since this will
	 * avoid character encoding errors.
	 * </p>
	 * <p>
	 * The language in which to write the model is specified by the
	 * <code>lang</code> argument. Predefined values are "RDF/XML",
	 * "RDF/XML-ABBREV", "N-TRIPLE", "TURTLE", (and "TTL") and "N3". The default
	 ...
	 * The language in which to write the model is specified by the
	 * <code>lang</code> argument. Predefined values are "RDF/XML",
	 * "RDF/XML-ABBREV", "N-TRIPLE" and "N3". The default value is represented by
	 * <code>null</code> is "RDF/XML".
	 * </p>
	 * 
	 ...
	 * 
	 * @param out  The output stream to which the RDF is written
	 * @param base The base uri to use when writing relative URI's.
	 *             <code>null</code> means use only absolute URI's. This is used for
	 *             relative URIs that would be resolved against the document
	 *             retrieval URL. For some values of <code>lang</code>, this value
	 ...
	 *
	 * <p>
	 * The statement with the same subject, predicate and object as that supplied
	 ...
	 * <p>
	 * If more than one statement witht the given subject and property exists in the
	 * model, it is undefined which will be returned. If none exist, an exception is
	 * thrown.
	 * 
	 ...
	 */
	Statement getRequiredProperty(Resource s, Resource p);
	 ...
	 * @throws PropertyNotFoundException
	 */
	Statement getRequiredProperty(Resource s, Resource p, String lang);

	/**
	 * Answer a statement (s, p, ?O) from this model. If none exist, return null; if
	 * several exist, pick one arbitrarily.
	 * 
	 ...
	 */
	Statement getProperty(Resource s, Resource p);
	 ...
	 */
	Statement getProperty(Resource s, Resource p, String lang);

	/**
	 * An alias for <code>listResourcesWithProperty(Property)</code>, retained for
	 * backward compatibility. It may be deprecated in later releases.
	 */
	ResIterator listSubjectsWithProperty(Resource p);

	/**
	 * Answer an iterator [with no duplicates] over all the resources in this model
	 * that have property <code>p</code>. <code>remove()</code> is not implemented
	 * on this iterator.
	 */
	ResIterator listResourcesWithProperty(Resource p);

	/**
	 * An alias for <code>listResourcesWithProperty</code>, retained for backward
	 * compatibility. It may be deprecated in later releases.
	 */
	ResIterator listSubjectsWithProperty(Resource p, RDFObject o);

	/**
	 * Answer an iterator [with no duplicates] over all the resources in this model
	 * that have property <code>p</code> with value <code>o</code>.
	 * <code>remove()</code> is not implemented on this iterator.
	 */
	ResIterator listResourcesWithProperty(Resource p, Resource o);

	/**
	 * List all objects in a model.
	 * 
	 * @return an iterator over the objects. .remove() is not implemented on this
	 *         iterator.
	 */
	ResIterator listObjects();

	/**
	 * List all objects of a given property. .remove() is not implemented on this
	 * iterator.
	 * 
	 * @return an iterator over the objects
	 * @param p The predicate sought
	 */
	ResIterator listObjectsOfProperty(Resource p);
	 ...
	 * @param p The predicate sought
	 */
	ResIterator listObjectsOfProperty(Resource s, Resource p);

	/**
	 * Determine whether this model contains any statements with a given subject and
	 * property.
	 * 
	 * @return true if there exists within this model a statement with subject s and
	 *         property p, false otherwise
	 * @param s The subject sought (null for any).
	 * @param p The predicate sought (null for any).
	 * 
	 */
	boolean contains(Resource s, Resource p);

	/**
	 * determine if the RDFNode r appears in any statement of this model.
	 * (containsRDFNode is a horrible name, and in any case, even literals will be
	 * resources one day)
	 * 
	 * @param r the RDFNode to be searched for
	 * @return true iff r appears as some subject, predicate, or object
	 */
	boolean containsResource(RDFObject r);

	/**
	 * Determine if an (S, P, O) pattern is present in this model, with null allowed
	 * to represent a wildcard match.
	 * 
	 * @return true if the statement with subject s, property p and object o is in
	 ...
	 * 
	 */
	boolean contains(Resource s, Resource p, RDFObject o);
	...
	/**
	 * Determine if any of the statements returned by an iterator are contained in
	 * this model.
	 * 
	 * @param iter an iterator of the statements to be tested
	 * 
	 * @return true if any of the statements returns by iter are contained in this
	 *         model and false otherwise.
	 */
	boolean containsAny(StmtIterator iter);

	/**
	 * Determine if all of the statements returned by an iterator are contained in
	 * this model.
	 * 
	 * @param iter an iterator of the statements to be tested
	 * 
	...
	/**
	 * Determine if any of the statements in a model are also contained in this
	 * model.
	 * 
	 * @param model the model containing the statements to be tested
	 * 
	 * @return true if any of the statements in model are also contained in this
	 *         model and false otherwise.
	 */
	boolean containsAny(Model model);

	/**
	 * Determine if all of the statements in a model are also contained in this
	 * model.
	 * 
	 * @param model the model containing the statements to be tested
	 * 
	 * @return true if all of the statements in model are also contained in this
	 *         model and false otherwise.
	 */
	 ...
	 *
	 * <p>
	 * A statement is considered to match if the <CODE>test</CODE> method of s
	 * returns true when called on s.
	 * </p>
	 * 
	...
	StmtIterator listStatements(Resource s, Resource p, Resource o);
	...
	/**
	 * answer a ReifiedStatement that encodes _s_, belongs to this Model, and is a
	 * Resource with that _uri_.
	 */
	...
	/**
	 * answer an iterator delivering all the reified statements "in" this model that
	 * match the statement _st_.
	 */
	...
	/**
	 * Create a new, independant, model containing all the statements which are in
	 * both this model and another. As models are sets of statements, a statement
	 * contained in both models will only appear once in the resulting model. The
	 * new model need not be of the same type as either this model or the argument
	...
	/**
	 * Create a new, independant, model containing all the statements in this model
	 * which are not in another. The new model need not be of the same type as
	 * either this model or the argument model: typically it will be a memory-based
	 * model.
	 * 
	 * @return a new model containing all the statements in this model that are not
	 *         in the given model.
	...
	/**
	 * Test whether the given object <code>m</code> is a model that is equal to this
	 * model, which is true iff the underlying graphs are identical Java objects.
	 * This is not the same test as comparing whether two models have the same
	 * structure (i.e. contain the same set of statements). To test for strucutural
	...
	/**
	 * If transactions are supported, execute the command c within a transaction and
	 * return its result. If not, throw an UnsupportedOperationException.
	 * 
	 * @deprecated use {@link #calculateInTxn(Supplier)} or migrate to
	 *             {@link #executeInTxn(Runnable)}.
	 */
	@Deprecated
	Object executeInTransaction(Command cmd);

	/**
	 * Execute the runnable <code>action</code> within a transaction. If it
	 * completes normally, commit the transaction, otherwise abort the transaction.
	 */
	void executeInTxn(Runnable action);

	/**
	 * Execute the supplier <code>action</code> within a transaction. If it
	 * completes normally, commit the transaction and return the result, otherwise
	 ...
	 *
	 * <p>
	 * For efficiency reasons, some implementations may create models which which
	 * are dependent on others, i.e. a change in one model may cause a change in
	 * another. If this is the case this method will return false, otherwise it will
	 * return true.
	 * </p>
	 *
	 ...
	 * See <a href="http://www.w3.org/TR/rdf-concepts#section-Graph-syntax">RDF
	 * Concepts</a>.
	 * <p>
	 * Two models are isomorphic when each statement in one can be matched with a
	 * statement in the other. Statements which are identical match.
	 * </p>
	 *
	 * <p>
	 * Special treatment is given to anonymous nodes. A binding is a one to one
	 * mapping which maps each anonymous node in <code>this</code> model to an
	 * anonymous node in <code>model</code>. Two statements s1 and s2 match under a
	 * binding if s1.subject is anonymous and s2.subject is anonymous and the
	 * binding maps s1.subject to s2.subject.
	 * </p>
	 *
	 * <p>
	 * Two models are isomorphic if there is a binding that allows all the
	 * statements in one model to match a a statement in the other.
	 * </p>
	 * 
	 ...
	 *
	 * <p>
	 * Not all implementations of Model require this method to be called. But some
	 * do, so in general its best to call it when done with the object, rather than
...
//    public ModelLock getModelLock() ;
	...
	/**
	 * Get the model lock for this model. See also the convenience operations
	 ...
	 * [in whole or in part].
	 * <p>
	 * The same listener may be registered many times; if so, it's methods will be
	 * called as many times as it's registered for each event.
	 * 
	...
	/**
	 * Unregister a listener from model-changed events on this model. The listener
	 * is dtached from the model. The model is returned to permit cascading. If the
	 ...
	 */
	public Model removeAll(Resource s, Resource p, Resource r);
