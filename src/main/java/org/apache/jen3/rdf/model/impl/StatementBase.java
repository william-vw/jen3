...
/**
 * Abstract base class for StatementImpl - pulls up the stuff that doesn't
 * depend on how statements are represented (as S/P/O or as Triples).
 */
public abstract class StatementBase {
	...
	/**
	 * replace this StaementImpl [ie the one of which this is the base] with (S, P,
	 * n). Answer the new StaementImpl. Abstract here to allow methods to be pulled
	 * up.
	 */
	protected abstract StatementImpl replace(RDFObject n);

	public abstract Resource get(byte position);
	...
	public abstract Property getPredicateProperty();
	 ...
	 */
	protected Resource mustBeResource(RDFObject n) {
	...
	/**
	 * utility: check that node is a Property, throw otherwise
	 */
	protected Property mustBeProperty(RDFObject n) {
		if (n instanceof Property)
			return (Property) n;
		else
			throw new NodeTypeRequiredException(n, "property");
	}
	...
	/**
	 * Answer a string describing <code>object</code>, quoting it if it is a
	 * literal.
	 */
	protected String objectString(RDFObject object) {
