...
import org.apache.jen3.rdf.model.impl.*;
import org.apache.jen3.reasoner.*;
import org.apache.jen3.shared.*;
import org.apache.jen3.util.ResourceUtils;
import org.apache.jen3.util.iterator.*;
import org.apache.jen3.vocabulary.*;
    ...
    implements OntResource
{
     ...
     * {@link org.apache.jen3.rdf.model.Resource#as as()} instead.
     */
     ...
     */
    @Override
    public ExtendedIterator<Resource> listSeeAlso() {
     ...
     */
    @Override
    public ExtendedIterator<Resource> listIsDefinedBy() {
     ...
     */
    @Override
    public ExtendedIterator<Resource> listLabels( String lang ) {
        checkProfile( getProfile().LABEL(), "LABEL" );
    ...
    public boolean hasLabel( Literal label ) {
        ...
        ExtendedIterator<Resource> i = listLabels( label.getLanguage() );
     ...
     */
    @Override
    public ExtendedIterator<Resource> listComments( String lang ) {
        checkProfile( getProfile().COMMENT(), "COMMENT" );
    ...
    public boolean hasComment( Literal comment ) {
        ...
        ExtendedIterator<Resource> i = listComments( comment.getLanguage() );
        while (!found && i.hasNext()) {
    ...
    public int getCardinality( Property p ) {
        int n = 0;
        for (Iterator<RDFObject> i =listPropertyValues( p ).filterKeep( new UniqueFilter<RDFObject>());  i.hasNext(); n++) {
        	  i.next();
        }
     ...
     * value, encoded as an Resource.  Maintains the invariant that there is
     ...
     * {@link #addProperty( Property, Resource ) addProperty}.
     * </p>
     *
     * @param property The property to update
     * @param value The new value of the property as an Resource, or null to
     *              effectively remove this property.
     */
    @Override
    public void setPropertyValue( Property property, Resource value ) {
     ...
     * if it doesn't have one.  The value is returned as an Resource, from which
     ...
     * @return An Resource whose value is the value, or one of the values, of the
     *         given property. If the property is not defined the method returns null.
     */
    @Override
    public Resource getPropertyValue( Property property ) {
     ...
     * value in the iterator will be an Resource, representing the value (object) of
    ...
    */
    ...
    public void remove() {
        ...
        {
            if ( s.getPredicateProperty().equals( first ) && s.getObject().equals( this ) )
            {
     ...
     */
    @Override
    public void removeProperty( Property property, Resource value ) {
    ...
    protected String selectLang( StmtIterator stmts, String lang ) {
        ...
        while (stmts.hasNext()) {
            Resource n = stmts.nextStatement().getObject();
    ...
    protected <T extends Resource> T objectAs( Property p, String name, Class<T> asClass ) {
        checkProfile( p, name );
        try {
    ...
    protected <T extends Resource> ExtendedIterator<T> listAs( Property p, String name, Class<T> cls ) {
        checkProfile( p, name );
    ...
    protected void addPropertyValue( Property p, String name, Resource value ) {
        checkProfile( p, name );
        addProperty( p, value );
    }

    /** Set the property value, checking that it is supported in the profile */
    protected void setPropertyValue( Property p, String name, Resource value ) {
        checkProfile( p, name );
        removeAll( p );
        addProperty( p, value );
    }
    ...
    protected boolean hasPropertyValue( Property p, String name, Resource value ) {
        checkProfile( p, name );
        return hasProperty( p, value );
    }

    /** Add the given value to a list which is the value of the given property */
    protected void addListPropertyValue( Property p, String name, Resource value ) {
        checkProfile( p, name );

        // get the list value
        if (hasProperty( p )) {
            Resource cur = getRequiredProperty( p ).getObject();
            ...
            addProperty( p, ((OntModel) getModel()).createList( new Resource[] {value.asResource()} ) );
        }
    }

    /** Convert this resource to the facet denoted by cls, by adding rdf:type type if necessary */
    protected <T extends Resource> T convertToType( Resource type, String name, Class<T> cls ) {
    ...
    protected void removePropertyValue( Property prop, String name, Resource value ) {
        checkProfile( prop, name );
        getModel().remove( this, prop, value );
    }

    /** Answer the given node presenting the OntResource facet if it can */
    private static Resource asOntResource( Resource n ) {
        return n.isURIOrAnon() ? n.as( OntResource.class ) : n;
    }
