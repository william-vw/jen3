...
import org.apache.jen3.assembler.assemblers.AssemblerGroup;
import org.apache.jen3.assembler.exceptions.AmbiguousSpecificTypeException;
...
import org.apache.jen3.shared.BadDescriptionMultipleRootsException;
import org.apache.jen3.shared.BadDescriptionNoRootException;
...
public class AssemblerHelp
    ...
    */
    ...
    private static Class<?> loadClassNamedBy( Statement s )
    {
        ...
        if ( x.startsWith("com.hp.hpl.jena") ) {
            String x1 = x.replaceFirst("com.hp.hpl.jena", "org.apache.jen3") ;
    ...
    */
    public static Set<Resource> findSpecificTypes( Resource root, Resource baseType )
        {
        List<Resource> types = root.listProperties( RDF.type ).mapWith( Statement::getObject ).toList();
    ...
    private static boolean hasNoCompetingSubclass( List<Resource> types, Resource candidate )
        {
            for ( RDFObject type : types )
            {
    ...
    */
    public static String getString( Statement s )
        {
        RDFObject ob = s.getObject();
        if (ob.isURIOrAnon()) throw new BadObjectException( s );
