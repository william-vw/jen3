...
import org.apache.jen3.assembler.*;
import org.apache.jen3.assembler.exceptions.*;
...
public abstract class AssemblerBase implements Assembler
{
    ...
    protected static RDFObject getUnique( Resource root, Property property )
        {
        List<Resource> nodes = root.listProperties( property ) .mapWith( s -> s.getObject() ).toList();
    ...
    */
    public static String getOptionalClassName( Resource root, Property classProperty )
        {
        RDFObject classNode = getUnique( root, classProperty );
        return
            classNode == null ? null
            : classNode.isLiteral() ? classNode.asNode().getLiteralLexicalForm()
            : classNode.isURIOrAnon() ? mustBeJava( classNode.asNode().getURI() )
            : null
            ;
        }
