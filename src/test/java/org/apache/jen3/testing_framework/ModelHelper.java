...
package org.apache.jen3.testing_framework;
...
public class ModelHelper extends GraphHelper {
	...
	protected static String nice(RDFObject n) {
	...
	public static Statement statement(String fact) {
		...
		RDFObject obj = rdfNode(st.nextToken());
		return builderModel.createStatement(sub, pred, obj);
	}

	public static Statement statement(Resource s, Property p, RDFObject o) {
		return builderModel.createStatement(s, p, o);
	}

	public static RDFObject rdfNode(Model m, String s) {
		return m.asRDFObject(NodeCreateUtils.create(s));
	}

	public static RDFObject rdfNode(String s) {
		return rdfNode(builderModel, s);
	}

	public static <T extends RDFObject> T rdfNode(String s, Class<T> c) {
	...
	public static Model modelAdd(Model m, String facts) {
		...
		while (semis.hasMoreTokens()) {
			...
			Resource sub = resource(st.nextToken());
			Property pred = property(st.nextToken());
			Resource obj = resource(st.nextToken());
			m.add(sub, pred, obj);
		}
