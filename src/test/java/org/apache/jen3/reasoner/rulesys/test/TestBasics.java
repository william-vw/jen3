...
import org.apache.jen3.reasoner.rulesys.BasicForwardRuleReasoner;
...
import org.apache.jen3.reasoner.rulesys.impl.BFRuleContext;
...
public class TestBasics extends TestCase  {
    ...
    public void testClauseMaching() {
        ...
        BasicForwardRuleInfGraph inf = new BasicForwardRuleInfGraph(
                                            new BasicForwardRuleReasoner(rules), rules, null, null);
        ...
        boolean match = RuleUtil.match(p1, new Triple(n1, n2, n3), env);
        assertTrue(!match);
        ...
        match = RuleUtil.match(p1, new Triple(n2, n1, n3), env);
        assertTrue(match);
        ...
        match = RuleUtil.match(p2, new Triple(n1, n2, n2), env);
        assertTrue(!match);
        assertEquals(n2, env.getEnvironment()[0]);
        assertEquals(n3, env.getEnvironment()[1]);
        assertEquals(null, env.getEnvironment()[2]);
        ...
        match = RuleUtil.match(p2, new Triple(n3, n1, n2), env);
        assertTrue(match);
        assertEquals(n2, env.getEnvironment()[0]);
        assertEquals(n3, env.getEnvironment()[1]);
    ...
    public void testEmbeddedFunctors() {
        ...
        RDFObject flit = infModel.getResource(R1.getURI()).getRequiredProperty(rbr).getObject();
        assertNotNull(flit);
