...
public class TestRDFNodes extends AbstractModelTestBase
{
	...
	{
		final Model m = ModelHelper.modelWithStatements(this, "");
		Assert.assertEquals(true, m.createResource("eh:/foo").isURIOrAnon());
		Assert.assertEquals(true, m.createResource().isURIOrAnon());
		Assert.assertEquals(false, m.createTypedLiteral(17).isURIOrAnon());
		Assert.assertEquals(false, m.createTypedLiteral("hello").isURIOrAnon());
	}

	public void testIsURIResource()
	{
		final Model m = ModelHelper.modelWithStatements(this, "");
		Assert.assertEquals(true, m.createResource("eh:/foo").isURI());
		Assert.assertEquals(false, m.createResource().isURI());
		Assert.assertEquals(false, m.createTypedLiteral(17).isURI());
	...
	public void testRDFNodeAsLiteral()
	{
		...
		Assert.assertSame(l, ((RDFObject) l).asLiteral());
	}
	...
	public void testRDFNodeAsResource()
	{
		final Model m = ModelHelper.modelWithStatements(this, "");
		final Resource r = m.createResource("eh:/spoo");
		Assert.assertSame(r, ((RDFObject) r).asResource());
	}
	...
	public void testRDFVisitor()
	{
		...
		final RDFObject S = m.createResource();
		final RDFObject P = m.createProperty("eh:PP");
		final RDFObject O = m.createLiteral("LL");
		/* */
