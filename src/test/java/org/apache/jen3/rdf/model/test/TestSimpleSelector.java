...
public class TestSimpleSelector extends AbstractModelTestBase
{
	...
	{
		final StmtIterator iter = model.listStatements(new SimpleSelector(null,
				RDFS.label, (RDFObject) null));
		int i = 0;
		while (iter.hasNext())
		{
			i++;
			final Statement stmt = iter.nextStatement();
			Assert.assertEquals(RDFS.label, stmt.getPredicateProperty());
		}
		Assert.assertEquals(2, i);
	}
	...
	public void testFindPropertyAndObject()
	{
		...
		while (iter.hasNext())
		{
			i++;
			final Statement stmt = iter.nextStatement();
			Assert.assertEquals(RDF.value, stmt.getPredicateProperty());
	...
	{
		Assert.assertTrue(new SimpleSelector(null, null, (RDFObject) null)
				.isSimple());
	}
	...
	{
		Assert.assertFalse(new SimpleSelector(null, null, (RDFObject) null) {
		}.isSimple());
	}
