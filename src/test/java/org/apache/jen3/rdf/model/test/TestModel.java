...
import org.apache.jen3.rdf.model.test.helpers.ModelHelper;
import org.apache.jen3.rdf.model.test.helpers.TestingModelFactory;
...
public class TestModel extends AbstractModelTestBase
{
	...
	{
		final RDFObject S = model.getRDFObject(NodeCreateUtils.create("_Blank"));
	...
	{
		final RDFObject S = model.getRDFObject(NodeCreateUtils.create("42"));
	...
	{
		RDFList list = model.createList(new ArrayList<Resource>().iterator());
	...
	{
		final RDFObject S = model.getRDFObject(NodeCreateUtils.create("spoo:S"));
		JenaTestBase.assertInstanceOf(Resource.class, S);
	...
	public void testGetProperty()
	{
		...
		final RDFObject ob = x.getProperty(ModelHelper.property(model, "P"))
				.getObject();
	...
	private void testPresentAsRDFNode( final Node n,
			final Class<? extends RDFObject> nodeClass )
	{
		final RDFObject r = model.asRDFObject(n);
		...
		for (final String[] case1 : cases)
		{
			for (int j = 0; j < 3; j += 1)
			{
				...
				final Resource S = (Resource) (s.equals(Node.ANY) ? null : mc
						.getRDFObject(s));
				final Property P = ((p.equals(Node.ANY) ? null : mc.getRDFObject(
						p).as(Property.class)));
				final Resource O = o.equals(Node.ANY) ? null : mc.getRDFObject(o).asResource();
	...
	public void testToStatement()
	{
		...
		Assert.assertEquals(GraphTestBase.node("P"), s.getPredicateProperty().asNode());
